...
F4共感AIチャットの中核となる5フェーズプロンプト設計。傾聴→共感→深掘り→洞察→行動提案の遷移ロジックをプロンプト内のシステム指示で制御する。4体系診断データ・過去会話要約・腑落ちマーカー・アクション記録のコンテキスト注入テンプレートも本タスクで設計する。MVPの心臓部であり、PMF成功基準（7日以内再訪率30%以上）を支える最重要レイヤー。
